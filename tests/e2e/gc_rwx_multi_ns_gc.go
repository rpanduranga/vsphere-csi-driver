...
var _ = ginkgo.Describe("[rwm-csi-tkg] Volume Provision Across Namespace", func() {
	...
	*/
	ginkgo.It("[rwm-csi-tkg] Verify static volume provisioning works across Namespace in the same GC", func() {
		...
		err = verifyVolumeMetadataInCNS(&e2eVSphere, volumeID, pvclaim.Name,
			persistentvolumes[0].ObjectMeta.Name, "")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())

		//TODO: Add a check to validate CnsVolumeMetadata crd
		...
		defer func() {
			...
			// Delete PVC in SVC.
			svcPVC, err := svClient.CoreV1().PersistentVolumeClaims(svcNamespace).Get(ctx, volHandle,
				metav1.GetOptions{})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
