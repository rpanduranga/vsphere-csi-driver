...
var _ = ginkgo.Describe("File Volume Test for Reclaim Policy", func() {
	...
	*/
	ginkgo.It("[rwm-csi-tkg] Verify Reclaim Policy Retain with file volume", func() {
		...
		framework.Logf("PVC name in SV " + volumeID)
		...
		defer func() {
			...
			// Delete PVC in SVC.
			svcPVC, err := svClient.CoreV1().PersistentVolumeClaims(svcNamespace).Get(ctx, volHandle, metav1.GetOptions{})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			...
			//Delete FCD
			err = e2eVSphere.deleteFCD(ctx, volumeID, defaultDatastore.Reference())
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		}()
