...
var _ = ginkgo.Describe("[rwm-csi-tkg] File Volume Provision with Statefulsets", func() {
	...
	/*
		...
		10. Scale-up statefulset to 3 replicas
		...
		for _, sspod := range ssPodsBeforeScaledown.Items {
			...
			for _, volumespec := range sspod.Spec.Volumes {
				if volumespec.PersistentVolumeClaim != nil {
					...
					volHandle := persistentvolume.Spec.CSI.VolumeHandle
					gomega.Expect(volHandle).NotTo(gomega.BeEmpty())
					volumeID := getVolumeIDFromSupervisorCluster(volHandle)
					gomega.Expect(volumeID).NotTo(gomega.BeEmpty())

					volumesBeforeScaleDown = append(volumesBeforeScaleDown, volHandle)
					...
					verifyCNSFileAccessConfigCRDInSupervisor(ctx, f, sspod.Spec.NodeName+"-"+volHandle,
						crdCNSFileAccessConfig, crdVersion, crdGroup, true)

					cnsFileAccessConfigCRDList = append(cnsFileAccessConfigCRDList, sspod.Spec.NodeName+"-"+volHandle)
		...
		for _, sspod := range ssPodsAfterScaleUp.Items {
			...
			for _, volumespec := range pod.Spec.Volumes {
				if volumespec.PersistentVolumeClaim != nil {
					...
					gomega.Expect(contains(volumesBeforeScaleDown, volHandle)).To(gomega.BeTrue())
